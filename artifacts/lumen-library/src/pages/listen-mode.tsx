...
export default function ListenModePage() {
  ...
  return (
    ...
    >
      ...
      {/* Header */}
      <header className="relative z-10 p-6 flex justify-between items-center gap-4">
        <Link
          href={`/book/${book.id}`}
          className="inline-flex items-center text-sm text-muted-foreground hover:text-primary transition-colors bg-background/60 backdrop-blur-sm px-4 py-2 rounded-full border border-border/40"
        >
          <ArrowLeft className="w-4 h-4 mr-2" />
          Back
        </Link>

        <div className="text-center bg-background/60 backdrop-blur-sm px-6 py-2 rounded-full border border-border/40">
          <h2 className="text-sm font-serif font-medium leading-tight">{book.title}</h2>
          <p className="text-xs text-muted-foreground leading-tight">{book.author}</p>
        </div>

        <div className="flex items-center gap-2">
          <span className="hidden md:inline-flex items-center gap-1.5 text-xs text-muted-foreground bg-background/60 backdrop-blur-sm px-3 py-1.5 rounded-full border border-border/40">
